...
describe('Validate Operators', () => {
    ...
    test('pipe', () => {
        const fn = pipe(fnNumToString, fnConcatString('0'), fnStrToInt);
    ...
    test('pipe 2', () => {
        const n = 2;
        const fn = pipe(fnAddOne, fnAddOne);
        const samples = [1, 2, 3, 4, 5];
        expect([...fn(samples)]).toEqual(samples.map(fnAdd(n)));
    });
    ...
    test('pipe 3', () => {
        const n = 3;
        const fn = pipe(fnAddOne, fnAddOne, fnAddOne);
        const samples = [1, 2, 3, 4, 5];
        expect([...fn(samples)]).toEqual(samples.map(fnAdd(n)));
    });
    ...
    test('pipe 4', () => {
        const n = 4;
        const fn = pipe(fnAddOne, fnAddOne, fnAddOne, fnAddOne);
        const samples = [1, 2, 3, 4, 5];
        expect([...fn(samples)]).toEqual(samples.map(fnAdd(n)));
    });
    ...
    test('pipe 5', () => {
        const n = 5;
        const fn = pipe(fnAddOne, fnAddOne, fnAddOne, fnAddOne, fnAddOne);
        const samples = [1, 2, 3, 4, 5];
        expect([...fn(samples)]).toEqual(samples.map(fnAdd(n)));
    });

    test('pipe different types', () => {
        const fn = pipe(fnAddOne, fnNumToString, fnConcatString('0'), fnConcatString('1'), fnStrToInt);
        const samples = [1, 2, 3, 4, 5];
    ...
    test('pipe returns the item if a function is falsy', () => {
        ...
        const fn = pipe(0 as any, null as any, undefined as any, false as any, NaN as any, fnAddOne);
        const samples = [1, 2];
